...
def get_pilot_and_data_indexes(fftsize:int|None=None, guard_size:int|None=None, pilot_step:int=10) -> tuple(np.ndarray):
    '''Generate indexes for pilots and data based on fftsize, guard_size and range between pilots (pilot_step)'''
...
def put_data_to_indexes(pilot_indexes:np.ndarray, data_indexes:np.ndarray, pilots:np.ndarray, data:np.ndarray) -> np.ndarray:
    '''Place pilots and data symbols to numpy array based on indexes generated by "get_pilot_and_data_indexes" function'''
...
def get_data_from_indexes(pilot_indexes:np.ndarray, data_indexes:np.ndarray, data:np.ndarray) -> tuple(np.ndarray):
    '''Get pilots and data symbols from numpy array based on indexes generated by "get_pilot_and_data_indexes" function'''

    pilots = data[pilot_indexes]
    user_data = data[data_indexes]

    return pilots, user_data
